...
ENV HOME /root
RUN (cd /var/tmp && git clone -b docker_release https://github.com/appsembler/configuration)
RUN docker -H unix:///docker.sock run -i -t -d -v /var/tmp/configuration:/configuration phusion/baseimage /sbin/my_init --enable-insecure-key
