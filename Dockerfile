...
RUN ssh -i insecure_key root@$IP apt-get update
RUN ssh -i insecure_key root@$IP apt-get upgrade -y && apt-get install -y openssl ca-certificates build-essential software-properties-common python-software-properties curl git-core libxml2-dev libxslt1-dev libfreetype6-dev python-pip python-apt python-dev
RUN ssh -i insecure_key root@$IP pip install --upgrade pip && pip install --upgrade virtualenv
